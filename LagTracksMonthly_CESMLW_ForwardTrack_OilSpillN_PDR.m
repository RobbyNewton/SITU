%  LagTracksMonthly_CESMLW_ForwardTrack_OilSpillN
%
%  A scrip to:
...
for imem = 1:length(member)
    ...
    for timeslice = 1:length(jstarts)
        ...
        % Last possible year of tracking
        jend            = YearMonth2JMonth_PDR(2100,12) ;

        % Declaring initial matrices
        % XLag & YLag: rows = individual grid cells in LIA; cols = months of run
        XLag            = NaN(numel(xstart),10*12+1) ; % start with 10 years of tracking, will go dynamically if needed
        YLag            = XLag ;
        XLag(:,1)       = xstart ;
        YLag(:,1)       = ystart ;
        ...
        %% Looping from jstart to jend (Dec 2100)
        for jmonth = jstart:jend-1 
            ...
            if ~isempty(Ice) % Skip if no still-active ice floes
                ...
                XLag(Ice,n+1) = XLag(Ice,n) + uwork*fac ; 
                YLag(Ice,n+1) = YLag(Ice,n) + vwork*fac ;
            else % exit loop if no active parcels left
                break
            end
...
%                             disp('Safe!')
        ...
        % Because XLag and YLag might grow dynamically (which assigns
        % zero to rows not active), replace all zeros by NaN
        XLag(XLag==0) = NaN ;
        YLag(YLag==0) = NaN ;

        %% Save data
        save(['Data_ForwardTrack_OilSpillN_CESMLW_' member{imem} '_' int2str(syear) '-' ...
            sprintf('%02d',smonth) 'toMelt.mat'],'XLag','YLag')
        clear XLag YLag

        movefile(['Data_ForwardTrack_OilSpillN_CESMLW_' member{imem} '_' int2str(syear) '-' ...
            sprintf('%02d',smonth) 'toMelt.mat'],CESMLWLIAMPath)
