%  LagTrackMonthly_CESMLE_ForwardTrack_LIA_PDR
%
%  A scrip to:
%   * Load the coordinates of the newly formed ice found using
%   EEZ_CESMLE_NewIce_Monthly_PDR.m
...
for imem = 1:length(member)
    ...
    % When working on Cheyenne
    load([CESMLEEEZMPath 'Formation/Matrix_NewIce_CESMLE_' member{imem} '.mat'])
    
%     % When working on Mac
%     load(['/Users/pade7652/Google Drive/CUBoulder/Projects/EEZFuture/Data/' ...
%         'CESMLE/monthly/Formation/Matrix_NewIce_CESMLE_' member{imem} '.mat'])
    ...
    %% Save data
    save(['Data_ForwardTrack_' int2str(nyears) 'yrs_LIA_CESMLE_' member{imem} ...
        '.mat'],'XLag','YLag','time','insideLIA')
    clear XLag YLag time insideLIA

    movefile(['Data_ForwardTrack_' int2str(nyears) 'yrs_LIA_CESMLE_' member{imem} ...
        '.mat'],CESMLELIAMPath)
    toc
